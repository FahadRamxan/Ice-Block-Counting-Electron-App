...
async function findFreePort(startPort: number, maxTries = 25): Promise<number> {
...
function appendFlaskLog(chunk: string): void {
  flaskLogTail += chunk;
...
function venvBaseInterpreterMissing(root: string): boolean {
 ...
 */
function resolvePythonExe(root: string): string | null {
  const embed = path.join(root, 'python-runtime', 'python.exe');
  if (app.isPackaged) {
    return fs.existsSync(embed) ? embed : null;
  }

  const venvPython = process.platform === 'win32'
    ? path.join(root, 'venv', 'Scripts', 'python.exe')
    : path.join(root, 'venv', 'bin', 'python');
  if (fs.existsSync(venvPython) && !venvBaseInterpreterMissing(root)) {
    return venvPython;
  }
...
function launchFlaskProcess(port: number): boolean {
  flaskLogTail = '';
...
async function waitForBackendHttp(port: number, timeoutMs: number): Promise<boolean> {
...
function createWindow(): void {
  mainWindow = new BrowserWindow({
    title: `Ice Factory Block Counter v${app.getVersion()}`,
    width: 1280,
    height: 800,
    webPreferences: {
...
app.whenReady().then(async () => {
  ...
  if (!launched) {
    dialog.showErrorBox(
      ...
      'Could not find the Python backend (backend/run_flask.py). Re-install the application.',
    );
    createWindow();
    return;
  }
  const waitMs = app.isPackaged ? 240000 : 90000;
